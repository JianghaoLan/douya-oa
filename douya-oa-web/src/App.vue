<template>
  <div id="app">
    <el-button type="primary" icon="el-icon-check" @click="deleteToken()" size="small">清除Token</el-button>
    <router-view />
  ...
  methods: {
    wechatLogin() {
      ...
      if(token === '' && openId != '') {
        ...
        if (token == '') {
          ...
          window.location = 'http://152.136.126.147:8800/admin/wechat/authorize?returnUrl=' + url
        }
      }
    },
    ...
    saveBind() {
      ...
      userInfoApi.bindPhone(this.bindPhoneVo).then(response => {
        ...
        window.location = 'http://152.136.126.147:9090/'
      })
    },
